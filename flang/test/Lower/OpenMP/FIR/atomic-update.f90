...
! RUN: bbc --use-desc-for-alloc=false %openmp_flags -emit-fir -hlfir=false %s -o - | FileCheck %s
! RUN: %flang_fc1 -mllvm --use-desc-for-alloc=false -emit-fir -flang-deprecated-no-hlfir %openmp_flags %s -o - | FileCheck %s
