...
/// We traverse through the use-def chain of tensor.parallel_insert_slice
/// through the containing scf.forall to fetch the first untiled consumer. We
/// also return the operand number of the consumer and the result number of the
/// scf.forall being consumed.
static Operation *getUntiledConsumerFromSliceDestSCFForall(
    tensor::ParallelInsertSliceOp candidateSliceOp, unsigned &operandNumber,
    unsigned &resultNumber) {
  // Step 1. Fetch the corresponding output
  ...
  Value resultingValue =
      iterArg.getOwner()->getParentOp()->getResult(resultNumber);
...
tileAndFuseConsumerOfSliceSCFForall(
  ...
  unsigned operandNumber = 0;
  unsigned resultNumber = 0;
  Operation *consumerOp = getUntiledConsumerFromSliceDestSCFForall(
      candidateSliceOp, operandNumber, resultNumber);
  if (!consumerOp)
    return failure();
  // Check that the consumer results in exactly one value.
  ...
  if (!checkAssumptionForFusingConsumer(forallOp.getResult(resultNumber))) {
